...
function main() {
  ...
  for (const [index, test] of SELFCHECK_LIST.entries()) {
    ...
    try {
      ...
      results.push({ name: test.name, status: 'FAIL', error: error?.message || 'Command failed' });
      failedCount += 1;
  ...
  console.log(`All ${SELFCHECK_LIST.length} system integrity self-checks passed successfully!`);
}
